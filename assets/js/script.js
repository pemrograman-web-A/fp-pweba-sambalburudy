...
// --- DATA PRODUK ---
const products = [
    { id: 1, name: "Sambal Bawang", price: 28000, img: "https://via.placeholder.com/400?text=Sambal+Bawang" },
    { id: 2, name: "Udang Crispy", price: 100000, img: "https://via.placeholder.com/400?text=Udang+Crispy" },
    { id: 3, name: "Spikoe Resep Kuno", price: 125000, img: "https://via.placeholder.com/400?text=Spikoe" },
    { id: 4, name: "Almond Crispy", price: 65000, img: "https://via.placeholder.com/400?text=Almond" },
    { id: 5, name: "Pecel Madiun", price: 25000, img: "https://via.placeholder.com/400?text=Pecel" },
    { id: 6, name: "Sambal Bajak", price: 30000, img: "https://via.placeholder.com/400?text=Sambal+Bajak" }
];
...
function updateNav() {
    ...
    if (navAuth) {
        if (activeUser) {
            ...
            navAuth.innerHTML = `
                <button onclick="openModal('login')" class="bg-red-700 text-white px-5 py-2 rounded-full font-bold text-sm hover:bg-red-800 shadow-md transition">
                    Masuk / Daftar
                </button>`;
        }
    }
...
function handleAuth(e) {
    ...
    if (isRegisterMode) {
        ...
        if (usersDB.find(user => user.username === u)) return alert("Username sudah dipakai!");

        const newUser = { username: u, password: p, name: name, email: u+"@mail.com", phone: "-", address: "-" };
...
function renderProducts() {
    ...
    grid.innerHTML = products.map(p => `
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 group hover:shadow-xl transition duration-300 flex flex-col h-full">
            ...
            <button onclick="addToCart(${p.id})" class="w-full bg-white border border-red-600 text-red-600 py-2 rounded-lg font-bold hover:bg-red-600 hover:text-white transition active:scale-95">
                + Keranjang
            </button>
        </div>
    `).join('');
...
async function checkOngkirAPI() {
    ...
    document.getElementById('ongkir-val').innerText = `Rp ${shippingCost.toLocaleString()}`;
    document.getElementById('val-ongkir').innerText = `Rp ${shippingCost.toLocaleString()}`;
...
async function payWithMidtrans() {
    ...
    const confirmPay = confirm(`Total Tagihan: ${document.getElementById('val-total').innerText}\nLanjutkan Pembayaran?`);
...
function toggleMode() { isRegisterMode = !isRegisterMode; renderModalUI(); }
function renderModalUI() {
    ...
    if (isRegisterMode) {
        title.innerText = "Daftar Akun Baru"; btn.innerText = "Daftar"; fieldName.classList.remove('hidden');
        toggle.innerHTML = `Sudah punya akun? <button onclick="toggleMode()" class="text-red-700 font-bold hover:underline">Login</button>`;
    } else {
        title.innerText = "Masuk Akun"; btn.innerText = "Masuk"; fieldName.classList.add('hidden');
...
function loadProfileData() {
    const activeUser = localStorage.getItem('buRudyActiveUser');
    if (!activeUser) { alert("Anda belum login!"); window.location.href = 'index.html'; return; }
    const user = JSON.parse(activeUser);
...
function saveProfile(e) {
    ...
    currentUser.name = newName; currentUser.phone = newPhone; currentUser.address = newAddress;
    ...
    if(idx !== -1) { usersDB[idx] = currentUser; localStorage.setItem('buRudyUsersDB', JSON.stringify(usersDB)); }
